...
import { createExchangeStateStore } from "./store.js";

dotenv.config();

const port = Number(process.env.PORT ?? "3002");
const webhookSecret = process.env.WEBHOOK_SECRET;
const storagePath = process.env.STORAGE_PATH ?? "./data/exchange-processor.json";

if (!webhookSecret) {
  throw new Error("Missing WEBHOOK_SECRET");
}

const store = createExchangeStateStore({ storagePath });
const app = express();
...
app.post(
  ...
  (req, res) => {
    try {
      ...
      store.setLastLifecycle(depositId, lifecycle);

      if (lifecycle === "completed" && !store.hasCredited(depositId)) {
        // In a real system, you would credit the user's account here.
        store.credit(depositId);
        // eslint-disable-next-line no-console
        console.log(`credited deposit ${depositId}`);
      }
...
app.listen(port, () => {
  // eslint-disable-next-line no-console
  console.log(`exchange-processor listening on :${port}`);
});
